...
def choice_prediction():
    st.write('## Bienvenue sur notre outil de classification.')
    st.write("Choisissez le fichier audio de l'animal marin de votre choix (au format .wav)")
    ...
    if uploaded_file is not None:
        ...
        if st.button('Predict'):
            ...
            st.write("Le mammifère marin détecté est : ",  str(classification(transform_wav_to_csv(sound))).replace('[', '').replace(']', '').replace("'", '').replace('"', ''))

    else:
        st.write("Le fichier n'a pas encore été chargé")
...
if __name__ == '__main__':
    ...
    st.write('___')
    st.image(Image.open('intro.png'))
    
    # create a sidebar
    st.sidebar.title('Menu')
    select = st.sidebar.selectbox('', ['Home', 'Prédiction'], key='1')
    st.sidebar.write(select)
    
    # if sidebar selection is "Prediction"
    if select=='Prédiction':
        # choice_prediction function
        choice_prediction()
    
    # else: stay on the home page
    else:
        st.write('# Mammifères marins')
        st.write("Notre outil est aujourd'hui capablede reconnaître le sons des mammifères marins suivants.")
        st.write("Pour plus d'informations, suivez ce [lien](https://cis.whoi.edu/science/B/whalesounds/index.cfm).")
